...
void DrawComponents(StructComponent *component, int number_components){
  for(int i = 0; i < number_components; i++){
    ...
    for (size_t j = 0; j < component[i].port_count; j++) {
      ...
      /* fprintf(svg_file, "<rect x='%d' y='%d' width='2' height='2' style='fill:red;stroke:none'/>\n", component[i].ports[j].x, component[i].ports[j].y); */
      /* fprintf(svg_file,"<rect x='%d' y='%d' width='1' height='1' style='fill:yellow;stroke:none'/>\n",125,160); */
      if(strcmp(component[i].ports[j].direction, "in") == 0){
	fprintf(svg_file, "<text class='label' x='%d' y='%d' text-anchor='start'>%s</text>\n", component[i].ports[j].x + 12, component[i].ports[j].y+8, component[i].ports[j].name);
      }else{
	fprintf(svg_file, "<text class='label' x='%d' y='%d' text-anchor='end'>%s</text>\n", component[i].ports[j].x-3, component[i].ports[j].y+8, component[i].ports[j].name);
      }
    }
  }
}
